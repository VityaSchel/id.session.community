<script lang="ts">
  ...
  let value = $derived(hash.substring(1))
  ...
  let valueSpecified = $derived(Boolean(value))
  let valueType = $derived(
    valueSpecified
      ? /^05[a-f0-9]{64}$/.test(value)
        ? 'sessionId'
        : onsRegex.test(value)
          ? 'ons'
          : null
...
<svelte:head>
  {#if valueSpecified}
    {#if valueType !== null}
      <title>Chat with {value}</title>
    {:else}
      <title>Invalid Session ID or ONS</title>
    {/if}
  {:else}
      ...
      {#if valueType !== null}
        ...
        >
          <ol class="list-decimal space-y-3 sm:space-y-4">
            ...
            <button
              class="block w-full cursor-pointer border border-black/20 bg-black/10 p-2 text-left font-mono text-sm break-all select-all"
              onpointerdown={() => {
                copy(value)
                copied = true
              }}
